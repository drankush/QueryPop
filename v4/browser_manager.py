    ...
    def __init__(self):
        class NewWindowBrowser(webbrowser.GenericBrowser):
            def open(self, url, new=2, autoraise=True):
                ...
                # Fetch and format browser window size from config
                try:
                    ...
                    print("Using default window size: {100, 50, 400, 700}")
                ...
                if sys.platform == 'darwin':  # macOS
                    ...
                    '''
                    chrome_result = subprocess.run(["osascript", "-e", chrome_script], stderr=subprocess.PIPE)

                    # If Chrome is not found, try Safari
                    if chrome_result.returncode != 0:  # Chrome likely not found
                        safari_script = f'''
                        ...
                        '''
                        safari_result = subprocess.run(["osascript", "-e", safari_script], stderr=subprocess.PIPE)
