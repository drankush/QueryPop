...
def show_popup(selected_text):
    ...
    new_width = initial_width // 8  # Adjust width for image
    ...
    instruction_entry = Text(frame, width=70, height=3)  # Use Text widget for taller input field
    ...
    # Calculate the maximum button text length based on the instruction labels
    max_label_length = max(
        len(f"[{key}] {instruction.split(':')[0]}") for key, instruction in config.INSTRUCTION_PROMPTS.items()
    )

    # Add some padding to the button width
    ...
    for key, instruction in config.INSTRUCTION_PROMPTS.items():
        ...
        # Create the button with the specified width and padding
